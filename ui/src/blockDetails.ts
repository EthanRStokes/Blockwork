...
const INSTRUCTION_EXPLAINERS: Record<InstructionType, string> = {
  ...
  WhenClipboardChanged: "Runs this strand the moment the system clipboard's contents change.",
  ...
  SetClipboard: 'Sets the system clipboard to the given text, replacing whatever it held before.',
...
const VALUE_KIND_LABELS: Partial<Record<ValueKind, string>> = {
  ...
  Round: 'Round', Math: 'Math Function', Random: 'Pick Random', Join: 'Join', Join3: 'Join (3)',
  ...
  ClipboardText: 'Clipboard Text', ClipboardHasImage: 'Clipboard Has Image', ClipboardHasFiles: 'Clipboard Has Files',
...
const VALUE_KIND_EXPLAINERS: Partial<Record<ValueKind, string>> = {
  ...
  Math: 'Applies the selected Scratch-style math function to a number. Trigonometric functions use degrees.',
  ...
  ClipboardText: 'The current contents of the system clipboard, as text.',
  ClipboardHasImage: 'True if the system clipboard currently holds image data.',
  ClipboardHasFiles: 'True if the system clipboard currently holds a list of files.',
